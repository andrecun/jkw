          ...
          <div class="container presidenRI-container">
               ...
               <div class="row presidenRI-contentDetail">
                    <div class="col-md-8 contentLeft">
                         <p class="title">Peraturan Perundangan (Peraturan Pemerintah)</p>
                         <?php
                         $alamat_web = "$url_rewrite" . "index.php/peraturan-pemerintah";
                         ...
                         $sql = "select *, DAYOFMONTH(waktu) as tgl, MONTH(waktu) as bln, "
                                 ...
                                 . "  from uu where status='1' and kategori='1' and waktu like '$tgl_sistem%' order by waktu desc ";
                         else
                              $sql = "select *, DAYOFMONTH(waktu) as tgl, MONTH(waktu) as bln, "
                                 . "YEAR(waktu) as thn, DATE_FORMAT(waktu, '%H:%i:%s') as jam"
                                 . "  from uu where status='1' and kategori='1' order by waktu desc ";
                      //   echo $sql;
                         $data = $DB->_fetch_array($sql, 1);
                         if(count($data)>0)
                         foreach ($data as $key => $value) {
                              $clsPath->addPathVarTo("peraturan-pemerintah", true);
