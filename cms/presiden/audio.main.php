	...
	<tr>
		<td>
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="60"><img src="/images/news.png" border="0" alt="<?php echo"Audio Streaming"; ?>" /></td>
					<td><font size="3"><b><?php echo"Audio Streaming"; ?></b></font></td>
				</tr>
			</table>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					<td class="menu"><b><?php echo"Tema Siaran"; ?></b></td>
				...
				<?php
				//$sqllimit = mysql_query("select jumlah from perhalaman where halaman='admin_wawancara'");
				//$rowlimit = mysql_fetch_array($sqllimit);
				//$limit = $rowlimit['jumlah'];
				$limit = 10;
				
				//if ($_GET['cat']!="") {
				//	$target = "file=audio_main&amp;cat=".$_GET['cat'];
				//	$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from audio where kategori='".$_GET['cat']."' order by waktu desc";
				//} else {
					$target = "file=audio_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from audio order by waktu desc";
				//}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<?php if($row[bahasa]==0) echo"$row[judul_id]"; else {?>
					<?php echo"$row[judul_en]"; }?></div></td>
					...
					<td align="center">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=audio_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=audio_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=audio_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";
					if(($levelku==1)||(!$row[status])&&($levelku==2))
					{
					echo "<a href=\"?file=audio_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus Audio Streaming &quot;$row[judul_id]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>
