...
<?php
$max_PerPem=$objconn->dbQueryGetRow('SELECT MAX(nomor) as nomor from uu where kategori=1;');
$max_PerPres=$objconn->dbQueryGetRow('SELECT MAX(nomor) as nomor from uu where kategori=2;');
$max_KepPres=$objconn->dbQueryGetRow('SELECT MAX(nomor) as nomor from uu where kategori=3;');
$max_InPres=$objconn->dbQueryGetRow('SELECT MAX(nomor) as nomor from uu where kategori=4;');
?>
var max_PerPem=<?php echo ($max_PerPem[nomor]+1); ?>;
var max_PerPres=<?php echo ($max_PerPres[nomor]+1); ?>;
var max_KepPres=<?php echo ($max_KepPres[nomor]+1); ?>;
var max_InPres=<?php echo ($max_InPres[nomor]+1); ?>;
function select_nomor()
	{
	if(uu.kategori.value==1)
		uu.nomor.value = max_PerPem;
	if(uu.kategori.value==2)
		uu.nomor.value = max_PerPres;
	if(uu.kategori.value==3)
		uu.nomor.value = max_KepPres;
	if(uu.kategori.value==4)
		uu.nomor.value = max_InPres;
	}
</script>
			...
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						...
						<form name="uu" action="./?file=uu_add" method="post" enctype="multipart/form-data">
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								...
								<td width="10">:</td>
								<td>
									<select class="inputtext" name="kategori" onChange="select_nomor()">
										<option value="" selected="selected">Pilih Kategori</option>
										<option value="">-------</option>
										<option value="1">Peraturan Pemerintah</option>
										<option value="2">Peraturan Presiden</option>
										<option value="3">Keputusan Presiden</option>
										<option value="4">Instruksi Presiden</option>
									</select>
								</td>
							</tr>
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="nomor" size="4" value="" /></td>
							</tr>
							<tr>
								<td><?php echo"Tahun"; ?></td>
								<td>:</td>
								<td><select class="inputtext" name="tahun">
							...
							<tr>
								...
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="5" cols="60" name="tentang"><?php echo"$tentang"; ?></textarea></td>
							</tr>
							<tr>
								<td><?php echo"Tempat Penetapan"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="tempat" value="<?php echo"Jakarta"; ?>" size="40" /></td>
							</tr>
							...
							<tr>
								...
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="5" cols="60" name="ringkasan"><?php echo"$row[ringkasan]"; ?></textarea></td>
							</tr>
							<tr>
								<td><?php echo"Dokumen UU"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="file" name="Dokumen" value="<?php echo"$Dokumen"; ?>" size="40" maxlength="255" /></td>
							</tr>
							<tr>
								<td><?php echo"Status"; ?></td>
								<td>:</td>
								<td><?php if ($levelku=="1") { ?>
								<input type="radio" name="status" value="0" checked='checked'/><?php echo"$bhs_tpk_pending"; ?>
								<input type="radio" name="status" value="1"/> <?php echo"$bhs_tpk_publish"; ?>
<?php } else { echo"$bhs_tpk_pending"; } ?></td>
							</tr>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_add_uu" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
