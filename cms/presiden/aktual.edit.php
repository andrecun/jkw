...
<?php if (($levelku=="1") ||($levelku=="2") ){ ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from aktual where id='".$_GET['id']."'");
						...
						$tahun = $row[tahun];
						?>
						<form action="./?file=aktual_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="judul" value="<?php echo"$row[judul]"; ?>" size="100" /></td>
							</tr>
							<tr>
								<td><?php echo"$bhs_kli_url"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="url" value="<?php echo"$row[url]"; ?>" size="100" /></td>
							</tr>														
							<tr>
...
<?php if ($levelku!="1") { echo"$bhs_fks_pending";  } else { ?>
							...
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_aktual" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
