...
<?php if (($levelku=="1") || ($levelku=="2")) { ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from uu where id='".$_GET['id']."'");
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								...
								<td>
									<select class="inputtext" name="kategori">
										...
										<option value="">-------</option>
										<option value="1" <?php if($row[kategori]==1) echo "selected"; ?>>Peraturan Pemerintah</option>
										<option value="2" <?php if($row[kategori]==2) echo "selected"; ?>>Peraturan Presiden</option>
										<option value="3" <?php if($row[kategori]==3) echo "selected"; ?>>Keputusan Presiden</option>
										<option value="4" <?php if($row[kategori]==4) echo "selected"; ?>>Instruksi Presiden</option>
									</select>
								</td>
							</tr>
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="nomor" size="4" value="<?php echo $row[nomor]; ?>" /></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="5" cols="60" name="tentang"><?php echo $row[tentang]; ?></textarea></td>
							</tr>
							<tr>
								<td><?php echo"Tempat Penetapan"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="tempat" value="<?php echo $row[tempat]; ?>" size="40" /></td>
							</tr>
							...
							<tr>
								<td valign="top"><?php echo"Dokumen UU"; ?></td>
								<td valign="top">:</td>
								<td>								
								<?php
								...
								<?php 
									echo "<a href='/DokumenUU.php?id=".$_GET[id]."' target=\"_blank\">Dokumen tersedia dengan format $doktype</a>";
									}
									else echo "Dokumen Tidak Tersedia"; ?>
								<br />
								Ganti dengan Dokumen yang lain :<br />
								<input class="inputtext" type="file" name="Dokumen" size="30" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_uu" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
