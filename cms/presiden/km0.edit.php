...
<?php if (($levelku=="1") ||($levelku=="2") ){ ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from km0 where id='".$_GET['id']."'");
						...
						$row[tahun] = $row[tahun];
						?>
						<form action="./?file=km0_edit&amp;id=<?php echo $_GET['id']; ?>" method="POST" enctype="multipart/form-data">
 ...
 <?php if ($levelku!="1") { echo"$bhs_fks_pending";  } else { ?>
							...
							<!--<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="penulis" value="<?php echo"$row[penulis]"; ?>" size="60" /><br /></td>
							</tr>-->
							<!--
							<tr>
							...
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_km0" value="<?php echo"$bhs_submit"; ?>" />&nbsp;<input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
