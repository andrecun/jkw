...
<?php if (($levelku=="1") ||($levelku=="2") ||($levelku=="6")){ ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from pers where id='".$_GET['id']."'");
						...
						$tahun = $row[tahun];
						?>
						<form action="./?file=pers_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_prs_edit_pers"; ?></b></td>
							</tr>
							<?php if ($verify_pers!="") { ?>
							<tr>
							...
							<tr>
								...
								<td>
									<select class="inputtext" name="kategori">
										<option value="1" <?php if($row[kategori]==1) echo "selected"; ?>>Keterangan Pers Presiden</option>
										<option value="2" <?php if($row[kategori]==2) echo "selected"; ?>>Keterangan Pers Juru Bicara Presiden</option>
										<option value="3" <?php if($row[kategori]==3) echo "selected"; ?>>Pers Rilis</option>
									</select>
								</td>
							</tr>
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="tempat" value="<?php echo htmlentities($row[tempat]); ?>" size="44" /></td>
							</tr>
							<tr>
...
<?php if ($levelku!="1") { echo"$bhs_fks_pending";  } else { ?>
								...
								<?php 
									echo "<a href='/DokumenPers.php?id=".$_GET[id]."' target=\"_blank\">Dokumen tersedia dengan format $doktype</a>";
									}
							...
							<tr>
								...
								if($row[english]==0) echo "checked='checked'"; ?> onclick="if
                (this.checked) {
					document.getElementById('indonesia_option').style.display = 'block';
					document.getElementById('english_option').style.display = 'none';
                 }; return true" />Indonesia&nbsp;
								<input id="english_version" type="radio" name="english" value="1" <?php
								if($row[english]==1) echo "checked='checked'"; ?> onclick="if
                (this.checked) {
					document.getElementById('indonesia_option').style.display = 'none';
                    document.getElementById('english_option').style.display = 'block';
                 }; return true" />Inggris
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="judul_id" value="<?php echo htmlentities($row[judul_id]); ?>" size="120" /></td>
							</tr>
							<tr>
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								...
								<td>:</td>
								<td><input class="inputtext" type="text" name="judul_en" value="<?php echo htmlentities($row[judul_en]); ?>" size="60" /></td>
							</tr>
							<tr>
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_pers" value="<?php echo"$bhs_submit"; ?>" />&nbsp;<input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
