	...
	<tr>
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				if ($_GET['cat']!="") {
					$target = "file=fotorilis_main&amp;cat=".$_GET['cat'];
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from fotorilis where kategori='".$_GET['cat']."' order by waktu desc";
				} else {
					$target = "file=fotorilis_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from fotorilis order by waktu desc";
				}
...
?>
					...
					<td width="180">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=fotorilis_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=fotorilis_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=fotorilis_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";
					if(($levelku==1))
					{
					echo "<a href=\"?file=fotorilis_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus Foto Rilis &quot;$row[judul]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>
