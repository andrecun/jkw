	...
	<tr>
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<?php
				//$sqllimit = mysql_query("select jumlah from perhalaman where halaman='admin_kibar'");
				//$rowlimit = mysql_fetch_array($sqllimit);
				//$limit = $rowlimit['jumlah'];
				$limit = 10;
				
				if ($_GET['cat']!="") {
					$target = "file=kibar_main&amp;cat=".$_GET['cat'];
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from kibar where kategori='".$_GET['cat']."' order by waktu desc";
				} else {
					$target = "file=kibar_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from kibar order by waktu desc";
				}
...
?>
					...
					<td width="180">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=kibar_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=kibar_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=kibar_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";
					if(($levelku==1))
					{
					echo "<a href=\"?file=kibar_main&amp;action=delete&amp;id=";
					echo $row[id];
...
?></td>
				...
				?>
				<tr>
					<td colspan="6" height="30" align="right"><?php echo"$bhs_kb_total"; ?> : <?php echo"$total"; ?> <?php if (($total<="1")) {echo"$bhs_kb_record";} else {echo"$bhs_kb_records";} ?></td>
				</tr>
				<tr>
					<td colspan="6">
