...
<?php if (($levelku=="1") ||($levelku=="2") ){ ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from kibar where id='".$_GET['id']."'");
						...
						$tahun = $row[tahun];
						?>
						<form action="./?file=kibar_edit&amp;id=<?php echo $_GET['id']; ?>" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_kb_edit_kibar"; ?></b></td>
							</tr>
							<?php if ($verify_kibar!="") { ?>
							<tr>
							...
							<?php } ?>
							<tr>
								<td width="100"><?php echo"$bhs_kb_date"; ?></td>
								<td width="10">:</td>
								<td>
							...
							<tr>
								...
								<td>:</td>
								<td>
 <?php if ($levelku!="1") { echo"$bhs_kb_pending";  } else { ?>
								<input type="radio" name="status" value="0" <?php if($row[status]==0) echo "checked='checked'"; ?> /> <?php echo"$bhs_kb_pending"; ?>
...
<td><textarea class="inputtext" rows="25" cols="120" name="isi_id"><?php echo $row[isi_id]; ?></textarea></td>
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_kibar" value="<?php echo"$bhs_submit"; ?>" />&nbsp;<input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
