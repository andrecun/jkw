...
<?php if (($levelku=="1") ||($levelku=="2") ||($levelku=="7") ){ ?>
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from kliping where id='".$_GET['id']."'");
						...
						$tahun = $row[tahun];
                                                                                $isi= $row[isi_id];
						?>
						<form action="./?file=kliping_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_kli_edit_news"; ?></b></td>
							</tr>
							<?php if ($verify_news!="") { ?>
							<tr>
							...
							<?php } ?>
							<tr>
								<td width="100"><?php echo"$bhs_kli_date"; ?></td>
								<td width="10">:</td>
								<td>
							...
							<tr>
								<td><?php echo"$bhs_kli_title"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="judul" value="<?php echo htmlentities($row[judul]); ?>" size="100" /></td>
							</tr>
							<tr>
								<td><?php echo"$bhs_kli_url"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="url" value="<?php echo htmlentities($row[url]); ?>" size="100" /></td>
							</tr>														
							<tr>
								<td><?php echo"$bhs_kli_source"; ?></td>
								<td>:</td>
								<td><input class="inputtext" type="text" name="sumber" value="<?php echo htmlentities($row[sumber]); ?>" size="40" /></td>
							</tr>

							<tr>
								<td><?php echo"$bhs_kli_status"; ?></td>
								<td>:</td>
								<td>
<?php if ($levelku!="1") { echo"$bhs_fks_pending";  } else { ?>
								...
								<?php echo"$bhs_fks_publish"; ?> <?php } ?>
								</td>
							</tr>
                                          <tr>
								<td valign="top"><?php echo"Isi "; ?></td>
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="25" cols="120" name="isi_id"><?php echo nl2br(htmlentities($row[isi_id])); ?></textarea></td>
							</tr>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_kliping" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
