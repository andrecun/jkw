		...
		<td>
			...
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from topik where id='".$_GET['id']."'");
$row = mysql_fetch_array($sql);
$row[cuplikan_id]=htmlspecialchars($row[cuplikan_id]);
$row[cuplikan_en]=htmlspecialchars($row[cuplikan_en]);
$row[isi_id]=htmlspecialchars($row[isi_id]);
$row[isi_en]=htmlspecialchars($row[isi_en]);


/*
$row[cuplikan_id]=stripslashes($row[cuplikan_id]);
$row[cuplikan_en]=str_ireplace("<br />","\n",$row[cuplikan_en]);
$row[isi_id]=str_ireplace("<br />","\n",$row[isi_id]);
$row[isi_en]=str_ireplace("<br />","\n",$row[isi_en]);
*/					
						$detik = $row[detik];
						$menit = $row[menit];
						$jam = $row[jam];
						...
						$tahun = $row[tahun];
						?>
						<form action="./?file=topik_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_tpk_edit_topik"; ?></b></td>
							</tr>
							<?php if ($verify_topik!="") { ?>
							<tr>
							...
							<?php } ?>
							<tr>
								<td width="100"><?php echo"$bhs_tpk_date"; ?></td>
								<td width="10">:</td>
								<td>
...
<?php echo"$bhs_tpk_publish"; ?>
								<?php } elseif ($row[status]=="1") { ?>
									<input type="radio" name="status" value="0" /> <?php echo"$bhs_tpk_pending"; ?> <input type="radio" name="status" value="1" checked="checked" /> <?php echo"$bhs_tpk_publish"; ?>
<?php } 
} else {
if ($row[status]=="0")
	echo"$bhs_tpk_pending";
	elseif ($row[status]=="1")
		echo"$bhs_tpk_publish";
}								
...
?>
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							...
							<tr>
								<td width="100"><?php echo"$bhs_tpk_title"; ?></td>
								<td width="10">:</td>
								<td><input class="inputtext" type="text" name="judul_id" value="<?php echo htmlentities($row[judul_id]); ?>" size="60" /></td>
							</tr>
							<tr>
							...
							<tr>
								<td valign="top"><?php echo"$bhs_tpk_content"; ?></td>
								<td valign="top">:</td>
						...
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_topik" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
