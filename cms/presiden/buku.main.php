	...
	<tr>
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				//} else {
					$target = "file=buku_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from buku order by waktu desc";
				//}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<td width="180" align="center">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=buku_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=buku_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=buku_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";
					if(($levelku==1))
					{
					echo "<a href=\"?file=buku_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus Buku &quot;$row[judul_id]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>					<td width="10">&nbsp;</td>
