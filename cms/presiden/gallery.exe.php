...
if ($_POST['submit_add_gallery']) {
		...
		$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
					values ('$session','gallery','$userku','$ip',now(),'add gallery album','$album')";
	...
	if (is_uploaded_file($_FILES['FotoR']['tmp_name'])) {
		...
		$sql = "INSERT INTO `gallery_album` (
					...
					VALUES (
									 '$_POST[album]',
									 '".$_POST[thn]."-".$_POST[bln]."-".$_POST[tgl]." ".$_POST[jam].":".$_POST[menit].":".$_POST[detik]."',";
	if(strlen($FotoR)>0)
		$sql.= "					 $FotoR,
									 '".$_FILES[FotoR][type]."',";
		else $sql.= "'', '',";
...
if ($_POST['submit_edit_gallery']) {
		// begin log ************************************************************
		$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
					values ('$session','gallery','$userku','$ip',now(),'mengubah gallery album','$album')";
		$logrow = mysql_query($logsql) or die(mysql_error());
		// end log ************************************************************
	...
	if (is_uploaded_file($_FILES['FotoR']['tmp_name'])) {
		...
		$sql = "UPDATE `gallery_album` SET
									`album` = '$_POST[album]', 
									`waktu` = '".$_POST[thn]."-".$_POST[bln]."-".$_POST[tgl]." ".$_POST[jam].":".$_POST[menit].":".$_POST[detik]."', ";
	if(strlen($FotoR)>0)
		$sql.= "					 `FotoR` = $FotoR,
...
if ( $_POST['submit_upload_gallery'] && (($levelku=="1") || ($levelku=="2") || ($levelku=="3")) ) {
	if ($_GET[id]!=""){
		// begin log ************************************************************
		$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
					values ('$session','gallery','$userku','$ip',now(),'upload gallery pic','')";
		$logrow = mysql_query($logsql) or die(mysql_error());
		// end log ************************************************************
...
if (($_GET['action'] == "delete")&&($_GET['file'] == "gallery_main")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
				values ('$session','gallery','$userku','$ip',now(),'delete gallery album','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
...
if (($_GET['action'] == "status")&&($_GET['file'] == "gallery_main")&&($_GET['status'] == "0")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
				values ('$session','gallery','$userku','$ip',now(),'publish gallery','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
...
if (($_GET['action'] == "status")&&($_GET['file'] == "gallery_main")&&($_GET['status'] == "1")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,module,username,ip,logtime,act,object) 
				values ('$session','gallery','$userku','$ip',now(),'pending gallery','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
