	...
	<tr>
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				//} else {
					$target = "file=aktual_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from aktual order by waktu desc";
				//}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<td><div style="padding:5px 0px 5px 0px"><?php echo"$row[judul]"; ?></div></td>
					...
					<td width="180">&nbsp;&nbsp; &bull;&nbsp; <a href="?file=aktual_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;
<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=aktual_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=aktual_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&nbsp&bull;&nbsp; 
<?php
					}
					else echo "Edit&nbsp;&nbsp&bull;&nbsp;"; 
					if(($levelku==1)||(!$row[status])&&($levelku==2))
					{
					echo "<a href=\"?file=aktual_main&amp;action=delete&amp;id=";
					echo $row[id];
...
?></td>
				...
				?>
				<tr>
					<td colspan="6" height="30" align="right"><?php echo"$bhs_kli_total"; ?> : <?php echo"$total"; ?> data</td>
				</tr>
				<tr>
					<td colspan="6">
