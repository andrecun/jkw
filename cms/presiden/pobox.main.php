		...
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					<td class="menu" width="120"><b><?php echo"Periode Waktu"; ?></b></td>
					<td class="menu" width="160" align="center"><b><?php echo"Status"; ?></b></td>
					<td class="menu" colspan="3" align="center"height="30"><b><?php echo"Action"; ?></b></td>
				</tr>
				<tr>
					<td colspan="5" bgcolor="#dddddd"><img src="/images/x.gif" border="0" alt="" /></td>
				</tr>
				...
				//if ($_GET['cat']!="") {
				//	$target = "file=pobox_main&amp;cat=".$_GET['cat'];
				//	$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from pobox where kategori='".$_GET['cat']."' order by waktu desc";
				//} else {
					$target = "file=pobox_main";
					$sql = "select *, DATE_FORMAT(tglAwal, '%d/%m/%Y') as awal, DATE_FORMAT(tglAkhir, '%d/%m/%Y') as akhir  from pobox order by tglAwal desc";
				//}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<td width="130"><?php echo"$row[awal] - $row[akhir]"; ?></td>
					...
					<td align="center">&nbsp;&nbsp; &bull;&nbsp; <a href="?file=pobox_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;
<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=pobox_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					}
					if(($levelku==1)||(!$row[status])&&($levelku==2)||(!$row[status])&&($levelku==5))
					{
?><a href="?file=pobox_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&nbsp&bull;&nbsp; 
<?php
					}
					else echo "Edit&nbsp;&nbsp&bull;&nbsp;"; 
					if(($levelku==1)||(!$row[status])&&($levelku==2)||(!$row[status])&&($levelku==5))
					{
					echo "<a href=\"?file=pobox_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus PO BOX periode &quot;$row[awal] - $row[akhir]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>
