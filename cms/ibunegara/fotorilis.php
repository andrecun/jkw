...
<?php include("../ibunegara/pers.nav.php"); ?>
	...
	<tr>
		<td>
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="60"><img src="/images/banner.png" border="0" alt="<?php echo"Foto Rilis"; ?>" /></td>
					<td><font size="3"><b><?php echo"Foto Rilis"; ?></b></font></td>
				</tr>
			</table>
			<table class="tablebg2" width="100%" cellspacing="0" cellpadding="12">
				<tr>
					<td valign="top">
						...
						<form action="../ibunegara/?file=fotorilis" method="post" enctype="multipart/form-data">
						<table width="100%" border="0" cellspacing="0" cellpadding="2">
							<tr>
								<td><b><?php echo"Tambah Foto Rilis"; ?></b></td>
							</tr>
							<tr>
								<td>&nbsp;</td>
							</tr>
							...
							<tr>
								<td><input class="inputsubmit" type="submit" name="submit_upload_fotorilis" value="<?php echo"$bhs_submit"; ?>" />&nbsp;<input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
						<?php } else {} ?>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								<td>
									<ul>
										<li><a href="../ibunegara/?file=fotorilis">Terbaru...</a></li>
									<?php
									$sqlarc = mysql_query("select *,DATE_FORMAT(waktu, '%Y-%m') as arsip from fotorilis where status='1' group by arsip order by arsip desc");
									while ($rowarc=mysql_fetch_array($sqlarc)) {
										$arsip = explode("-", $rowarc[arsip]);
										$bln = $arsip[1]; $thn = $arsip[0];
									?>
										<li><a href="../ibunegara/?file=fotorilis&amp;bln=<?php echo"$bln"; ?>&amp;thn=<?php echo"$thn"; ?>"><?php conv_bln("$bln"); echo" $thn"; ?></a></li>
									<?php
									}
									?>
									</ul>
					...
					<td valign="top">
						...
						if (($_GET['bln']!="") && ($_GET['thn']!="")) {
							$sql = mysql_query("select *, DATE_FORMAT(waktu, '%d/%m/%Y') as waktu from fotorilis where DATE_FORMAT(waktu, '%m')='".$_GET['bln']."' and DATE_FORMAT(waktu, '%Y')='".$_GET['thn']."' order by waktu desc");
						} else {
							$sql = mysql_query("select *, DATE_FORMAT(waktu, '%d/%m/%Y') as waktu from fotorilis order by waktu desc limit 0,10");
						}
						?>
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><b><?php echo"Foto Rilis"; ?></b></td>
							</tr>
						...
						<?php while ($row=mysql_fetch_array($sql)) { ?>
							<tr>
								<td>
									<?php
									list($width, $height, $type, $attr) = @getimagesize("../../presiden/fotorilis/$row[pic]");
									...
									?>
									<img src="../../presiden/fotorilis/<?php echo"$row[pic]"; ?>" width="<?php echo"$panjang"; ?>" height="<?php echo"$lebar"; ?>" border="0" alt="<?php echo"$row[pic]"; ?>" /><br />
									<table width="100%"  border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td width="50">Tanggal</td><td width="10">:</td><td><?php echo"$row[waktu]"; ?></td>
										</tr>
										<tr>
