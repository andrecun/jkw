...
if ($_POST['submit_add_kliping']) {
	...
	//else {
		...
		$sql = "insert into kliping (waktu,judul,sumber,url,status) values ('".$_POST[thn]."-".$_POST[bln]."-".$_POST[tgl]." 00:00:00','$_POST[judul]','$_POST[sumber]','$_POST[url]','".($levelku==1?$_POST[status]:0)."')";
...
if ($_POST['submit_edit_kliping']) {
		// begin log ************************************************************
		$logsql = "insert into log (session,username,ip,logtime,act,object)
					values ('$session','$userku','$ip',now(),'edit kliping','')";
		$logrow = mysql_query($logsql) or die(mysql_error());
		// end log ************************************************************

		$sql = "update kliping set waktu='".$_POST[thn]."-".$_POST[bln]."-".$_POST[tgl]." 00:00:00', sumber='$_POST[sumber]', judul='$_POST[judul]', url='$_POST[url]', status='".($levelku==1?$_POST[status]:0)."' where id='".$_POST['id']."'";
		$result = mysql_query($sql) or die(mysql_error());
...
if (($_GET['action'] == "status")&&($_GET['file'] == "kliping_main")&&($_GET['status'] == "0")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,username,ip,logtime,act,object) 
				values ('$session','$userku','$ip',now(),'publish kliping','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
...
if (($_GET['action'] == "status")&&($_GET['file'] == "kliping_main")&&($_GET['status'] == "1")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,username,ip,logtime,act,object) 
				values ('$session','$userku','$ip',now(),'pending kliping','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
...
if (($_GET['action'] == "delete")&&($_GET['file'] == "kliping_main")) {
	// begin log ************************************************************
	$logsql = "insert into log (session,username,ip,logtime,act,object) 
				values ('$session','$userku','$ip',now(),'delete kliping','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
...
if ($_POST['submit_add_cat_kliping']) {
	...
	elseif ($kategori_en=="") { }
		...
		// begin log ************************************************************
		$logsql = "insert into log (session,username,ip,logtime,act,object) 
					values ('$session','$userku','$ip',now(),'add kliping category','')";
		$logrow = mysql_query($logsql) or die(mysql_error());
		// end log ************************************************************
...
if ($_POST['submit_edit_cat_kliping']) {
	...
	elseif ($kategori_en=="") { }
		...
		// begin log ************************************************************
		$logsql = "insert into log (session,username,ip,logtime,act,object) 
					values ('$session','$userku','$ip',now(),'edit kliping category','')";
		$logrow = mysql_query($logsql) or die(mysql_error());
		// end log ************************************************************
...
if ($_GET['mode'] == "submit_del_cat_kliping") {
	// begin log ************************************************************
	$logsql = "insert into log (session,username,ip,logtime,act,object) 
				values ('$session','$userku','$ip',now(),'delete kliping category','')";
	$logrow = mysql_query($logsql) or die(mysql_error());
	// end log ************************************************************
