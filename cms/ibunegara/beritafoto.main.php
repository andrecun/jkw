...
if($_GET["action"]=="delete")
	...
	$SQL="update beritafoto set status='$status' where id=".$_GET["id"];
	$objconn->dbQuery($SQL);
	}
	...
	<tr>
		<td>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<?php
				//$sqllimit = mysql_query("select jumlah from perhalaman where halaman='admin_beritafoto'");
				//$rowlimit = mysql_fetch_array($sqllimit);
				//$limit = $rowlimit['jumlah'];
				$limit = 10;
				
				if ($_GET['cat']!="") {
					$target = "file=beritafoto_main&amp;cat=".$_GET['cat'];
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from beritafoto where kategori='".$_GET['cat']."' order by waktu desc";
				} else {
					$target = "file=beritafoto_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from beritafoto order by waktu desc";
				}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<td colspan="2"><div style="padding:5px 0px 5px 0px"><?php echo"$row[judul]"; ?>
<?php
if($row[judul_en]!='')
	{
?>
					...
					<td width="180">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=beritafoto_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=beritafoto_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=beritafoto_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";

					if($levelku==1)
					{
					echo "<a href=\"?file=beritafoto_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus Berita Foto &quot;$row[judul_id]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>
