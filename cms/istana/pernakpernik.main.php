	...
	<tr>
		<td>
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="60"><img src="/images/news.png" border="0" alt="<?php echo"Pernak Pernik"; ?>" /></td>
					<td><font size="3"><b><?php echo"Pernak Pernik"; ?></b></font></td>
				</tr>
			</table>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				if ($_GET['cat']!="") {
					$target = "file=pernakpernik_main&amp;cat=".$_GET['cat'];
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from pernakpernik where kategori='".$_GET['cat']."' order by waktu desc";
				} else {
					$target = "file=pernakpernik_main";
					$sql = "select *, DATE_FORMAT(waktu, '%d/%m/%Y') as tanggal from pernakpernik order by waktu desc";
				}
				...
				<tr bgcolor="<?php echo"$tdcol"; ?>" onmouseover="setPointer(this, '#ADD6FF')" onmouseout="setPointer(this, '<?php echo"$tdcol"; ?>')">
					...
					<td colspan="2"><div style="padding:5px 0px 5px 0px"><?php echo"$row[judul_id]"; ?>
<?php
if($row[judul_en]!='')
	{
?>
					...
					<td width="180">&nbsp;&nbsp;&bull;&nbsp;<a href="?file=pernakpernik_detail&amp;id=<?php echo $row[id]; ?>">Detail</a>&nbsp;&bull;&nbsp;<?php
					if($levelku==1)
					{
					echo "<a href=\"?file=pernakpernik_main&amp;action=status&amp;id=";
					echo $row[id];
					echo "&amp;status=";
				    echo $row[status]; 
				    echo "\">Status</a> &bull;&nbsp;";
					...
					{
?><a href="?file=pernakpernik_edit&amp;id=<?php echo $row[id]; ?>">Edit</a>&nbsp;&bull;&nbsp;<?php
					}
					else echo "Edit&nbsp;&bull;&nbsp;";

					if($levelku==1)
					{
					echo "<a href=\"?file=pernakpernik_main&amp;action=delete&amp;id=";
					echo $row[id];
					echo "\" onclick=\"return confirm('Apakah Anda yakin untuk menghapus Pernak Pernik &quot;$row[judul_id]&quot;')\">Delete</a>";
					}else {
?>Delete<?php
						}
?></td>
