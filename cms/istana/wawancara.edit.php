			...
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						...
						$tahun = $row[tahun];
						?>
						<form action="../istana/?file=wawancara_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_wwn_edit_news"; ?></b></td>
							</tr>
							<?php if ($verify_news!="") { ?>
							<tr>
							...
							<?php } ?>
							<tr>
								<td width="100"><?php echo"$bhs_wwn_date"; ?></td>
								<td width="10">:</td>
								<td>
							...
							<tr>
								<td><?php echo"$bhs_wwn_category"; ?></td>
								<td>:</td>
								<td>
									<select class="inputtext" name="kategori">
										<option value=""><?php echo"$bhs_wwn_select_category"; ?></option>
										<option value="">---</option>
										<?php
							...
							<tr>
								<td valign="top"><?php echo"$bhs_wwn_content"; ?></td>
								<td valign="top">:</td>
							...
							<tr>
								<td><?php echo"$bhs_wwn_status"; ?></td>
								<td>:</td>
								<td>
								<?php if ($row[status]=="0") { ?>
									<input type="radio" name="status" value="0" checked="checked" /> <?php echo"$bhs_wwn_pending"; ?> <input type="radio" name="status" value="1" /> <?php echo"$bhs_wwn_publish"; ?>
								<?php } elseif ($row[status]=="1") { ?>
									<input type="radio" name="status" value="0" /> <?php echo"$bhs_wwn_pending"; ?> <input type="radio" name="status" value="1" checked="checked" /> <?php echo"$bhs_wwn_publish"; ?>
								<?php } ?>
								</td>
							</tr>
							<tr>
