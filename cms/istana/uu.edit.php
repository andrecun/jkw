		...
		<td>
			...
			<table class="tablebg2" width="100%" align="center" cellpadding="4" cellspacing="0">
				<tr>
					<td>
						<?php
						$sql = mysql_query("select *, DAYOFMONTH(waktu) as tanggal, MONTH(waktu) as bulan, YEAR(waktu) as tahun, SECOND(waktu) as detik, MINUTE(waktu) as menit, HOUR(waktu) as jam from download where id='".$_GET['id']."'");
						...
						$tahun = $row[tahun];
						?>
						<form action="../istana/?file=uu_edit&amp;id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
						<table width='100%' border='0' align='center' cellpadding='2' cellspacing='0'>
							<tr>
								<td colspan="3"><b><?php echo"$bhs_dwn_edit_download"; ?></b></td>
							</tr>
							...
							<?php } ?>
							<tr>
								<td width="100"><?php echo"$bhs_dwn_date"; ?></td>
								<td width="10">:</td>
								<td>
							...
							<tr>
								...
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="10" cols="120" name="isi_id"><?php echo"$row[isi_id]"; ?></textarea></td>
							</tr>
							<tr>
								<td valign="top"><?php echo"$bhs_dwn_content"; ?></td>
								<td valign="top">:</td>
								<td><textarea class="inputtext" rows="10" cols="120" name="isi_en"><?php echo"$row[isi_en]"; ?></textarea></td>
							</tr>
							<tr>
								...
								<td>
								<?php if ($row[status]=="0") { ?>
									<input type="radio" name="status" value="0" checked="checked" /> <?php echo"$bhs_dwn_pending"; ?> <input type="radio" name="status" value="1" /> <?php echo"$bhs_dwn_publish"; ?>
								<?php } elseif ($row[status]=="1") { ?>
									<input type="radio" name="status" value="0" /> <?php echo"$bhs_dwn_pending"; ?> <input type="radio" name="status" value="1" checked="checked" /> <?php echo"$bhs_dwn_publish"; ?>
								<?php } ?>
								</td>
							</tr>
							<tr>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><input class="inputsubmit" type="submit" name="submit_edit_download" value="<?php echo"$bhs_submit"; ?>" /> <input class="inputsubmit" type="reset" value="<?php echo"$bhs_reset"; ?>" /></td>
							</tr>
						</table>
						</form>
					</td>
