...
if ($_POST['submit_add_kliping']) {
	...
	//else {
		...
		$sql = "insert into kliping (waktu,judul_id,judul_en,cuplikan_id,cuplikan_en,isi_id,isi_en,sumber,url,status) values ('$thn-$bln-$tgl $jam:$mnt:$dtk','$judul_id','$judul_en','$cuplikan_id','$cuplikan_en','$isi_id','$isi_en','$sumber','$url','$status')";
...
if ($_POST['submit_edit_news']) {
	if ($kategori=="") { $verify_news= "$bhs_kli_category_empty"; }
	elseif (($judul_id=="") && ($judul_en=="")) { $verify_news = "$bhs_kli_title_empty"; }
	elseif (($isi_id=="") && ($isi_en=="")) { $verify_news = "$bhs_kli_content_empty"; }
	else {
		// begin log ************************************************************
		$logsql = "insert into log (session,username,ip,logtime,act,object)
		...
		$sql = "update kliping set waktu='$thn-$bln-$tgl $jam:$mnt:$dtk', sumber='$sumber', penulis='$userku', kategori='$kategori', judul_id='$judul_id', judul_en='$judul_en', isi_id='$isi_id', isi_en='$isi_en', status='$status' where id='".$_POST['id']."'";
		$result = mysql_query($sql) or die(mysql_error());
		
		$verify_news = "$bhs_kli_success_edit_news";
	}
}
if($levelku == 1 ) {
